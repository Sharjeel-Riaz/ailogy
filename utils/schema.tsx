...
export const AdminUser = pgTable(
  ...
  (table) => {
    return {
      userIdIdx: index("admin_user_idx").on(table.userId),
    };
  }
);

// Coursework Category Table
export const CourseworkCategory = pgTable(
  "courseworkCategory",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    isActive: varchar("isActive").default("true"),
    createdAt: timestamp("createdAt").defaultNow(),
    updatedAt: timestamp("updatedAt").defaultNow(),
  },
  (table) => {
    return {
      nameIdx: index("coursework_category_name_idx").on(table.name),
    };
  }
);

// Coursework Template Table
export const CourseworkTemplate = pgTable(
  "courseworkTemplate",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    formFields: text("formFields").default("[]"),
    isActive: varchar("isActive").default("true"),
    sortOrder: integer("sortOrder").default(0),
    createdBy: varchar("createdBy"),
    createdAt: timestamp("createdAt").defaultNow(),
  ...
  (table) => {
    return {
      slugIdx: index("coursework_slug_idx").on(table.slug),
      categoryIdx: index("coursework_template_category_idx").on(table.category),
    };
  }
);
